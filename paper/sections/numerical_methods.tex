...
|\psi|<10^{-12}.
\]
For comparison, the Supplementary Material reports both the adopted
single-coordinate displacement and the distance to the constrained nearest
point of $\psi=0$ in the $(c_1,c_2)$ plane when both coordinates are allowed
to vary. Every refined configuration is then checked for membership in the
...
validated against central finite differences at 25 test points chosen away
from the primary singularities. The analytic gradient
...
and an augmented-system residual not exceeding $10^{-11}$ is required.
