...
The compiled manuscript is included as [`paper/main.pdf`](paper/main.pdf), with
supplementary material in [`paper/supplement.pdf`](paper/supplement.pdf).
...
paper/                     LaTeX manuscript and compiled PDFs
  ...
  main.pdf, supplement.pdf compiled documents
...
Note: the `results/` directory (the CSV outputs) is **not** tracked. It is
...
5. **Targeted refinements** (optional)
   ...
   ```
   Resolve the near-period-doubling turn-back and search for a genuine
   subharmonic family at the one low-order multiplier resonance that is crossed.
